...
function CartListPage() {
    const { cartItems } = useContext(ShoppingCartContext); // Corrected from `catdItems` to `cartItems`
                ...
                <div className="md:col-span-2 space-y-4">
                    {
                        ...
                        cartItems.map(singleCartItem => (
                            <CartTile key={singleCartItem.id} singleCartItem={singleCartItem} /> // Added `key` prop
                        ))
                        : <h1>No items available in cart! Please add some items.</h1>
                    }
                </div>
                <div className="bg-gray-100 rounded-sm p-4 h-max">
                    <h3 className="text-xl font-extrabold text-gray-950 border-l-gray-300 pb-2">Order Summary</h3>
                    ...
                    <div className="mt-5 flex gap-2">
                        <button className="text-sm px-4 py-3 bg-black text-white font-extrabold disabled:opacity-60" disabled={cartItems.length === 0}>CheckOut</button>
